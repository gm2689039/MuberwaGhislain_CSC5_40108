/* 
  ...
  Created on February 6, 2017, 11:38 AM
  Purpose:  collect five students' information and scores from four test.
 *          Modification of problem 13 where the lowest grade is dropped 
 *          in calculating average grade.
 */
...
char averageGrade(double scores[]){
    ...
    double lowestScore;         //Hold the lowest grade
    
    //Calculate sum and find the lowest grade
    for(int i = 0; i < NUMBER_OF_TESTS; i++){
        if(i == 0){
    ...
    //Calculate average. Also remove lowest score and one test 
    average = (sum - lowestScore)/(NUMBER_OF_TESTS - 1);
